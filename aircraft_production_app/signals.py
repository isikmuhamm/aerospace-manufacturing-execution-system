...
from .models import Aircraft, WorkOrder, WorkOrderStatusChoices, Part, PartStatusChoices, AircraftStatusChoices # AircraftStatusChoices'ı import et
...
def update_work_order_status_on_aircraft_creation(sender, instance, created, **kwargs):
    if instance.work_order:  # Eğer uçak bir iş emrine bağlıysa
        ...
        # Sadece durumu "İptal Edildi" olmayan iş emirlerini güncelle
        if work_order.status != WorkOrderStatusChoices.CANCELLED:
            completed_aircraft_count = Aircraft.objects.filter(
            ...
            # durum PENDING veya ASSIGNED'a geri dönebilir.
